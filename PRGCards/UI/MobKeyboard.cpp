...
void MobKeyboard::pointerReleaseEvent(MAPoint2d p)
{
	...
	{
		if (m_attachedWidget != NULL && l_notfull && ((((MobEditBox*) m_attachedWidget)->getInputMode() != MobEditBox::IM_NUMBERS)
				||(((MobEditBox*) m_attachedWidget)->getInputMode() == MobEditBox::IM_NUMBERS && (m_activeLayout==LT_NUM1||m_activeLayout==LT_NUM2))))
		{
			bool isPasswordMode = ((MobEditBox*) m_attachedWidget)->isPasswordMode();
			if (isPasswordMode) {
				((MobEditBox*) m_attachedWidget)->setPasswordMode(FALSE);
