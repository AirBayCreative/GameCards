...
Logout::Logout(Screen *previous, Feed *feed) : previous(previous), feed(feed) {
	mainLayout = createMainLayout(back, logout);
	listBox = (ListBox*) mainLayout->getChildren()[0]->getChildren()[2];
	MAExtent imgSize = maGetImageSize(RES_LOGOUT);
	int imgHeight = EXTENT_Y(imgSize);
	imge = new Image(0, 0, scrWidth-PADDING*2, imgHeight, listBox, false, false, RES_LOGOUT);
