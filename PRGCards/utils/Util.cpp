...
Label* createSubLabel(String str, int height) {
	Label *label = new Label(0, 0, scrWidth, height, NULL, str, 0, gFontBlackBold);
...
Widget* createSoftKeyBar(int height, const char *left, const char *right, const char *centre) {
	...
	Label *label = new Label(0,0, scrWidth/3 - (PADDING*2), height - PADDING, NULL, left, 0, gFontWhiteBold);
	label->setHorizontalAlignment(Label::HA_CENTER);
	label->setVerticalAlignment(Label::VA_CENTER);
	if (strlen(left) != 0) {
	...
	label = new Label(0,0, scrWidth/3 + (scrWidth%3), height - PADDING, NULL, centre, 0, gFontWhiteBold);
	label->setHorizontalAlignment(Label::HA_CENTER);
	label->setVerticalAlignment(Label::VA_CENTER);
	if (strlen(centre) != 0) {
		label->setSkin(gSkinButton);
	...
	label = new Label(0,0, scrWidth/3 - (PADDING*2), height - PADDING, NULL, right, 0, gFontWhiteBold);
	label->setHorizontalAlignment(Label::HA_CENTER);
	label->setVerticalAlignment(Label::VA_CENTER);
	if (strlen(right) != 0) {
		label->setSkin(gSkinButton);
...
Layout* createMainLayout(const char *left, const char *right, const char *centre, bool useKinetic) {
	...
	if (useKinetic) {
		KineticListBox *mKineticBox = new KineticListBox(0, 0, scrWidth, scrHeight-(softKeys->getHeight()+mBox->getHeight()),
				NULL, KineticListBox::LBO_VERTICAL, KineticListBox::LBA_LINEAR, false);
		listBox->add(mKineticBox);
	}
	else {
		ListBox *mListBox = new ListBox(0, 0, scrWidth, scrHeight-(softKeys->getHeight()+mBox->getHeight()),
