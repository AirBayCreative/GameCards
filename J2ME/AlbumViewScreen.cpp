...
AlbumViewScreen::AlbumViewScreen(Screen *previous, Feed *feed, String filename) : mHttp(this), filename(filename+ALBUMEND), previous(previous), feed(feed) {
	...
	int res = mHttp.create((CARDS+filename).c_str(), HTTP_GET);
...
void AlbumViewScreen::keyPressEvent(int keyCode) {
	...
	switch(keyCode) {
		...
		case MAK_FIRE:
			if (index.size() >- 1) {
				next = new ImageScreen(this, RES_TEMP, RES_TEMP, false, &cards.find(index[selected])->second, false, mImageCache);
				next->show();
			}
			break;
		case MAK_SOFTRIGHT:
