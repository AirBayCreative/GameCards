...
inline void SYSCALL(maCloseStore) (int store, int _remove) {
	SYSCALL_IMPL(maCloseStore)(convertSingleArg<MAHandle>(store), convertSingleArg<int>(_remove));
}
...
inline int SYSCALL(maInvokeExtension) (int function, int a, int b, int c) {
	int ret;
	convertRet<int>(ret, __dbl_high, (int) SYSCALL_IMPL(maInvokeExtension)(convertSingleArg<int>(function), convertSingleArg<int>(a), convertSingleArg<int>(b), convertSingleArg<int>(c)));
	return ret;
}
